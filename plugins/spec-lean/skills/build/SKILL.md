...
# /spec-lean:build — Phase 2: Implement the Spec
...
1. **Spec exists?** `docs/specs/$1/spec.md` must exist. If not, stop and tell the user to run `/spec-lean:spec $1`.
...
3. **`.gitignore` scaffolded?** If the project has no `.gitignore`, scaffold a minimal one for the language detected (look at the spec's Section 4 "Technical details" for the stack). Do this even when git isn't in use yet — a future `git init` then inherits the file and won't accidentally track build artifacts.
   ...
   If the spec's Section 4 names a runtime data file or DB under "Storage" (e.g. `data.json`, `*.db`), include it in `.gitignore` too. This prevents the `/spec-lean:ship` review from catching runtime artifacts as a CAUTION later.

4. **Git stash + branch (if git is in use).**
   - Run `git status --porcelain`. If the working tree is dirty, run `git stash push -u -m "spec-lean:build pre-start stash for $1"` and tell the user you stashed their WIP — do NOT silently bundle unrelated work into the spec branch.
...
3. After implementing, **self-review** the change against the story's acceptance: does the code actually deliver what the story describes? Run any quick local sanity check the Done-when block describes (a single `Bash` command, an import smoke check) — but do **not** run the full Done-when validation here; that's `/spec-lean:validate`'s job.
...
> Run `/spec-lean:validate $1` to walk through each story's Done-when checks.
...
- **Never `git push`, `--force`, `--no-verify`, or `git reset --hard`** in this flow. Commit happens in `/spec-lean:ship`.
